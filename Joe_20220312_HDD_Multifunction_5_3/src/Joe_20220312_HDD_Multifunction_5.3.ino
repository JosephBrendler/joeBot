...
void calculateClockFace()
{
  ...
  if ( newminutes == 0 ) ClockFace[newhours] = GREEN;    // write the new one
  if ( newseconds == 0 ) ClockFace[newminutes] = BLUE;   // write the new one
...
void loop_fn_clock()
{
  ...
  while ( ! ACTIVATED ) {
    ...
    if ( clockTick ) {
      ...
      if ( clockPosition < 60 ) {  // provides offset and reverses b/c disk rotates ccw
        PORTB = ClockFace[59 - ((offset + clockPosition) % 60)];
        delayMicroseconds(intervalOn);
        PORTB &= ~((1 << redLED) | (1 << greenLED) | (1 << blueLED));
      }
...
void loop_fn_radar() {
  ...
  while ( ! ACTIVATED ) {
    ...
    if ( clockTick ) {
      if ( ! DONE_RADAR ) {   // only once per period
        PORTB |= (1 << redLED);
        delayMicroseconds(intervalOn);
        PORTB &= ~(1 << redLED);
        DONE_RADAR = true;
